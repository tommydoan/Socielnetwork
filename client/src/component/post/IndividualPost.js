    ...
    if (comments) {
      return (
        <Fragment>
          <div
            style={{ padding: 2 }}
            className="container bg-primary py-2 pt-2 "
          >
            <div className="post bg-white p-1 my-1">
              <div>
                <img className="round-img" src={post.avatar} alt="beauty" />
                <h4>{post.name}</h4>
              </div>
              <div>
          ...
          <section style={{ marginTop: '0.5rem' }} className="container">
            {alert ? (
              ...
              <h4 className="muted">Leave a Comment</h4>
            )}
              ...
              <div class="post bg-white p-1 my-1">
                <div>
                  <Link to={`/profiles/${comment.user}`}>
                    <img class="round-img" src={comment.avatar} alt="beauty" />
                    <h4>{comment.name}</h4>
                  </Link>
                </div>
                <div>
                  ...
                  {user._id === post.user ? (
                    <Button
                      onClick={this.onClick.bind(this, comment._id)}
                      style={{ float: 'right' }}
                      color="danger"
                      sm
                    >
...
export default connect(
  mapStateToProps,
  { getIndividualPost, addComment, deleteComment }
)(IndividualPost);
